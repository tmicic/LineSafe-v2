...
MODEL_PATH = r'unet_for_self_sup_learn.model'
        ...
        for training, dataset, dataloader in [(True, train_dataset, train_dataloader), 
            ...
            with context_manager:
                ...
                for i, (X, y, cat) in enumerate(dataloader):
                    ...
                    # no delete y from X, and then set target to be X (self learning!)

                    y = torch.abs(y - 1.)   # invert y
                    
                    X = X * y               # MAP X[with NG blanked] to orig X (now target)
            ...
            if not training:
                ...
                ax1.imshow(target.view(-1,256).detach().cpu(), cmap='gray')
